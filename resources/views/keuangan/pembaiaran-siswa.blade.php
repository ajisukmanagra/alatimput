    ...
    <div class="card">
        <div class="card-body">
            ...
            Import 
             ...
             <a href="/keuangan/daftar-pembaiaran-siswa/create" class="btn btn-primary" target="_blank">
            ...
            <div class="modal fade" id="exampleModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
              <div class="modal-dialog">
                ...
                <table id="bangunanTable" class="table mt-4">
                    ...
                    <tbody>
                        @php $no = 1 @endphp
                        @foreach($pembairan as $k)
                        <tr>
                        <td>{{ $no ++ }}</td>
                        <td>{{ $k->nama }}</td>
                        <td>{{ $k->kelas }}</td>
                        <td>{{ $k->jenis_kelamin }}</td>
                        <td>{{ $k->jumlah_baiaran }}</td>
                        <td>
                           <a href="/keuangan/edit-data-pembairan-siswa/{{ $k->id }}" class="btn btn-warning">Ubah"</a>
                            <a href="/keuangan/pembaiaran-siswa/delete/{{ $k->id }}" class="btn btn-danger">Hapus</a>
                        </td>
                        </tr>
                        @endforeach
                    </tbody>
